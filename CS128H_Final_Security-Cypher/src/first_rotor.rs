pub mod first_rotor {
    ...
    lazy_static! {
        static ref FORWARD_MAPPING_FIRST_ROTOR: HashMap<char, char> = {
            let key_chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
            let value_chars = "EKMFLGDQVZNTOWYHXUSPAIBRCJ";
    ...
    lazy_static! {
        static ref BACKWARD_MAPPING_FIRST_ROTOR: HashMap<char, char> = {
            let key_chars = "EKMFLGDQVZNTOWYHXUSPAIBRCJ";
    ...
    pub fn forward_mapping_first_rotor(input: char) -> char {
        let output = FORWARD_MAPPING_FIRST_ROTOR.get(&input);
        return output;
    }

    pub fn backward_mapping_first_rotor(input: char) -> char {
        let output = BACKWARD_MAPPING_FIRST_ROTOR.get(&input);
        return output;
    }
}